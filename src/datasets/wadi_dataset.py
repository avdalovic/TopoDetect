...
class WADIDataset(Dataset):
    """Dataset class for WADI anomaly detection."""
    def __init__(self, data, wadi_complex, feature_dim_0=3, n_input=10, temporal_mode=False, temporal_sample_rate=1):
        """
        Initialize the WADI dataset for anomaly detection.
        ...
        wadi_complex : WADIComplex
            Combinatorial complex representing the WADI system topology
        ...
        self.feature_dim_2 = feature_dim_0 + 1  # 2-cells: 4D (avg of 0-cells + subsystem zone size)
        ...
        # Get sensor and actuator columns (excluding timestamp, label, and WADI-specific columns)
        ...
        self.columns = [col for col in data.columns if col not in exclude_columns]
    ...
    def preprocess_features(self):
        ...
        print("Preprocessing WADI features...")
        ...
        print(f"Preprocessed WADI features shape: {features.shape}")
        return features
    ...
    def compute_initial_x1(self):
        ...
        """
        print("Computing initial WADI 1-cell features (concatenation method)...")
        num_samples = len(self.data)
        ...
        print(f"Found {num_1_cells} WADI 1-cells")
        ...
        for cell_idx, cell in enumerate(cells_1):
            ...
            if len(boundary_nodes) != 2:
                print(f"Warning: WADI 1-cell {cell} has {len(boundary_nodes)} boundary nodes, expected 2")
                continue
            ...
            try:
                ...
                print(f"Warning: Couldn't find WADI node in row_dict: {e}")

        print(f"Computed WADI 1-cell features shape: {x_1.shape}")
        return x_1

    def compute_initial_x2(self):
        """
        Initialize 2-cell features based on grouped 0-cells.
        Add subsystem zone size information as 4th dimension.
        ...
        """
        print("Computing initial WADI 2-cell features (with subsystem zone size)...")
        num_samples = len(self.data)
        ...
        print(f"Found {num_2_cells} WADI 2-cells")
        ...
        for cell_idx, cell in enumerate(cells_2):
            ...
            try:
                ...
                if node_indices:
                    ...
                    print(f"Warning: No valid 0-cells found for WADI 2-cell {cell}")
                    # Set zone size to 0 if no valid nodes
                    x_2[:, cell_idx, -1] = 0.0

            except Exception as e:
                print(f"Warning: Error computing WADI 2-cell features for cell {cell}: {e}")
                print(f"Cell contents: {cell}")

        print(f"Computed WADI 2-cell features shape: {x_2.shape}")
        return x_2
    ...
    def print_sample_features(self):
        ...
        if self.x_2.shape[1] > 0:
            ...
            print(f"  - 4th dim (subsystem zone size): {sample_2_cell[3].item():.4f}")
