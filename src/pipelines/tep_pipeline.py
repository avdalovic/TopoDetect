...
from src.utils.topology.tep_topology import TEPComplex
from src.utils.attack_utils import get_attack_indices, get_attack_sds
from src.datasets.tep_dataset import TEPDataset
from src.models.ccann import AnomalyCCANN
from src.trainers.anomaly_trainer import AnomalyTrainer


def load_tep_data(train_path, test_path, sample_rate=1.0, save_test_path=None, validation_split_ratio=0.2, 
                  attack_focused_sampling=False, pre_attack_points=10, attack_start_points=5):
    """
    Load TEP dataset from CSV files with optional sampling and validation split.
    ...
    """
    print(f"Loading TEP data from {train_path} and {test_path}...")
    ...
    print(f"Loaded TEP data: train={len(initial_train_data)} rows, test={len(test_data)} rows")
    ...
    if 'Atk' not in initial_train_data.columns:
        ...
        initial_train_data['Atk'] = 0
    
    # Ensure test data has 'Atk' column
    if 'Atk' not in test_data.columns:
        print("Warning: Test data missing 'Atk' column, adding default (all normal)")
        test_data['Atk'] = 0
    ...
    if sample_rate < 1.0:
        ...
        if attack_focused_sampling:
            ...
            attack_indices, _ = get_attack_indices("TEP")
    ...
    normal_test = (test_data['Atk'] == 0).sum()
    attack_test = (test_data['Atk'] == 1).sum()
...
def run_experiment(config):
    """Main function to run TEP experiment based on a config dictionary."""
    print("Starting TEP Anomaly Detection Experiment (3-level)...")
    ...
    saved_test_path = os.path.join(saved_data_dir, f"tep_test_data_{config['data']['sample_rate']}.pkl")
    ...
    if os.path.exists(saved_test_path):
        ...
        # We call load_tep_data but ignore its test_data output and prevent it from saving.
        print("Loading corresponding training and validation data...")
        train_data, validation_data, _ = load_tep_data(
        ...
        train_data, validation_data, test_data = load_tep_data(
            train_path=train_path,
            test_path=test_path,
            sample_rate=config['data']['sample_rate'],
    ...
    # Load TEP topology
    print("Loading TEP topology...")
    tep_complex = TEPComplex()

    # Get component names
    component_names = [col for col in train_data.columns if col != 'Atk']
    ...
    train_dataset = TEPDataset(train_data, tep_complex, **dataset_args)
    validation_dataset = TEPDataset(validation_data, tep_complex, **dataset_args) if not validation_data.empty else None
    test_dataset = TEPDataset(test_data, tep_complex, **dataset_args)
    ...
    print("TEP experiment finished successfully!") 