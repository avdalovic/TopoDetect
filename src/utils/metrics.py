"""
Advanced, time-aware metrics for evaluating anomaly detection performance.
Based on "Precision and Recall for Time Series" by Tatbul et al.
"""
import numpy as np
...
def calculate_fp_alarms(y_true, y_pred, attacks, window_seconds=60, sample_hz=1):
    """
    Counts the number of continuous false positive alarms.
    ...
    Args:
        ...
        sample_hz (int): The sampling frequency of the data.
    ...
    window_points = window_seconds * sample_hz
...
def calculate_time_aware_metrics(y_true, y_pred, attacks, alpha=0.0, cardinality='reciprocal', bias='flat'):
    """
    Calculate time-aware precision and recall (eTaP, eTaR, eTaF1).
    Based on "Precision and Recall for Time Series" by Tatbul et al.
    ...
    Args:
        ...
        y_pred (np.array): Predicted labels.
        attacks (list of tuples): List of (start_index, end_index) for each attack.
        alpha (float): Bias parameter (0.0 = no bias towards early detection).
        cardinality (str): How to weight multiple detections ('reciprocal' or 'one').
        bias (str): Bias function ('flat', 'front', 'middle', 'back').
    ...
    # Calculate time-aware precision (eTaP)
    etap = calculate_existence_reward_precision(y_true, y_pred, anomaly_ranges, alpha, cardinality, bias)
    
    # Calculate time-aware recall (eTaR)
    etar = calculate_existence_reward_recall(y_true, y_pred, anomaly_ranges, alpha, cardinality, bias)
    
    # Calculate time-aware F1 (eTaF1)
    if etap + etar > 0:
...
def calculate_existence_reward_precision(y_true, y_pred, anomaly_ranges, alpha, cardinality, bias):
    """Calculate existence-reward precision (eTaP)."""
...
def calculate_existence_reward_recall(y_true, y_pred, anomaly_ranges, alpha, cardinality, bias):
    """Calculate existence-reward recall (eTaR)."""
    if not anomaly_ranges:
        return 0.0
...
def calculate_overlap_reward(pred_start, pred_end, true_start, true_end, alpha, cardinality, bias):
    """Calculate the overlap reward between predicted and true anomaly segments."""
    # Find overlap region
...
def calculate_bias_weight(overlap_start, overlap_end, true_start, true_end, bias):
    """Calculate bias weight based on position of overlap within true anomaly."""
    true_length = true_end - true_start + 1
